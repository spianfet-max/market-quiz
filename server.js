...
// Pre-warm only ALL/VP on startup (most common combo — 1 API call only)
async function warmCache() {
  console.log('[Cache] Pre-warming ALL/VP...');
  await generateAndCache('ALL', 'VP');
}

// Refresh only cached combos every 4 hours (lazy — only what's been used)
setInterval(() => {
  const keys = Object.keys(questionCache);
  console.log(`[Cache] Scheduled refresh for ${keys.length} cached combo(s)...`);
  keys.forEach((key, i) => {
    const [cat, diff] = key.split('_');
    setTimeout(() => generateAndCache(cat, diff), i * 15000);
  });
}, CACHE_TTL_MS);
